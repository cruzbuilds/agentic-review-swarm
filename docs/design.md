...
The history is meant to separate them in three: the tag holds V1 as tested; the tool-execution fix (`a18fba7`, the tagged commit itself) is a defect found while testing V1; everything on `v2/hierarchical-review-model` is V2. The second step landed late: the fix branch was never merged into `main`, the V2 branch was cut from `main` at `b4732ea` without it, and `scripts/review.sh` on V2 carried the `--allowedTools` line that denied every tool in Experiment 001's first condition until the merge at `745f155`, after the research record was consolidated. The evaluations were unaffected (`run-seeds.sh` has its own flags); no real review had been run through `review.sh` on the branch. Recorded in `docs/research/v2-hybrid-review.md` section 9.
